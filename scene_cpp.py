...
from cpp.fem_lib import Scene3d
...
class SceneC3d:
    ...
    def solve(self, external_force, time_step, method="ExplicitEuler", method_config={}):
        ...
        self.core.InitializeGridsAndVelocity([(float(j[0]), float(j[1]), float(j[2])) for j in self.XY],
                                             [(int(j[0]), int(j[1]), int(j[2]), int(j[3])) for j in self.M])
        ...
        if method == "ExplicitEuler":
            for t in tqdm(range(frame_len)):
                self.core.ComputeNodalForce([(float(j[0]),float(j[1]),float(j[2])) for j in external_force[t]])
                self.core.ForwardEuler(self.material["mass"], time_step)
